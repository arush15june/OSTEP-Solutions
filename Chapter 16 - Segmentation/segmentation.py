...
for vStr in addrList:
    ...
    if options.solve == False:
        print '  VA %2d: 0x%08x (decimal: %4d) (binary: %s)--> PA or segmentation violation?' % (i, vaddr, vaddr, bin(vaddr))
    else:
        paddr = 0
        if (vaddr >= (asize / 2)):
            # seg 1
            ...
            if paddr < base1:
                print '  VA %2d: 0x%08x (decimal: %4d) (binary: %s) --> SEGMENTATION VIOLATION (SEG1)' % (i, vaddr, vaddr, bin(vaddr))
            else:
                print '  VA %2d: 0x%08x (decimal: %4d) (binary: %s) --> VALID in SEG1: 0x%08x (decimal: %4d)' % (i, vaddr, vaddr, bin(vaddr), paddr, paddr)
        else:
            # seg 0
            if (vaddr >= len0):
                print '  VA %2d: 0x%08x (decimal: %4d) (binary: %s) --> SEGMENTATION VIOLATION (SEG0)' % (i, vaddr, vaddr, bin(vaddr))
            else:
                paddr = vaddr + base0
                print '  VA %2d: 0x%08x (decimal: %4d) (binary: %s) --> VALID in SEG0: 0x%08x (decimal: %4d)' % (i, vaddr, vaddr, bin(vaddr), paddr, paddr)
    i += 1
